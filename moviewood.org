...
- It is entirely possible to play the game without spending real
  ...
  occasionally ($1 per play). The "buy gold" and "buy money" options
  are NOT a good deal.
...
Moviewood is a hidden object / social game that is not brilliant, but
which I have found annoyingly compulsive, hence this FAQ.
...
Scenes are built using Movie Coins - a new Scene opens every other
(odd) level.
...
Also, sometimes scenes appear in their mirror image (no extra
points). Scenes can be updated with Props (see my Props FAQ) after
level 4. 

See my Scenes and Sets FAQ for a list of Scenes.
...
Sets are built using game cash. A new set opens every other (even)
level. You can sell Sets but you shouldn't - you only recoup a tiny
fraction of their cost, and they come in useful when you need
multiples of a specific set for a task or for a prop drop. 

See my Scenes and Sets FAQ for a list of Sets.
...
they drop of, see my Props FAQ

** Actors

For a list of available actors see my Actors FAQ
...
Flash hints are best for the regular leves; occasionally there are
...
You can put a smaller (eg 2x2) set in one of the 3x3 blocks for
...
ID is 94a0d4ebb061 (feel free to try it). If you play the game on
multiple devices (eg. iPhone and iPad) you will have different IDs and
can friend your other self. 

You cannot delete a friend once added. If there is a limit in how many
you can have, I have not yet hit it (currently at ~ 45).
...
'Joe123' I am going to try and give them something back later; if I
get one from 'Player' I have no idea which of the several 'Player's in
my friends list it came from.
...
Random Gift seems to go away after ~ Level 30 so make use of it while
you can. 
